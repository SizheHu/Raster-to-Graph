...
if __name__ == '__main__':
    ...
    files = list(Path('../output_r2v/eval').glob('*.pth'))
    # plot_precision_recall(files)
    # plt.show()
    # plot_logs(logs=Path('../output'),fields=('class_error', 'loss_bbox_unscaled', 'mAP'), ewm_col=0, log_name='log.txt')
    plot_logs(logs=Path('../output_r2v_3'), fields=('loss', 'loss_ce', 'loss_bbox'), ewm_col=0,
              log_name='log.txt')
    plt.show()
